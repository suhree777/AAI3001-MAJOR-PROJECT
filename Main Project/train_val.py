...
annotation_paths = [os.path.join(annotation_dir, f.replace(
    '.tif', '.xml')) for f in os.listdir(image_dir) if f.endswith('.tif')]
...
# Test Dataset
test_image_paths = [os.path.join(test_data_dir, f) for f in os.listdir(
    test_data_dir) if f.endswith('.tif')]
test_annotation_paths = [os.path.join(test_data_dir, f.replace(
    '.tif', '.xml')) for f in os.listdir(test_data_dir) if f.endswith('.tif')]
...
def main():
    check_disjoint_sets('train_images.json',
                        'val_images.json', 'test_images.json')
